            ...
            <div className="page-container">
                ...
                <div className="stats-grid">
                    <div className="stat-card">
                        ...
                        <div>
                            <div className="stat-value" style={{ color: 'var(--color-success)', fontSize: '1.25rem' }}>{formatCurrency(totalIncome)}</div>
                    ...
                    <div className="stat-card">
                        ...
                        <div>
                            <div className="stat-value" style={{ color: 'var(--color-danger)', fontSize: '1.25rem' }}>{formatCurrency(totalExpense)}</div>
                    ...
                    <div className="stat-card">
                        ...
                        <div>
                            <div className="stat-value" style={{ color: 'var(--color-info)', fontSize: '1.25rem' }}>{formatCurrency(totalIncome - totalExpense)}</div>
                            ...
                            <tbody>
                                {loading ? <tr><td colSpan={7} style={{ textAlign: 'center', padding: '3rem' }}>Memuat...</td></tr> :
                                    filtered.length === 0 ? <tr><td colSpan={7}><div className="empty-state"><DollarSign size={48} /><h3>Belum ada transaksi</h3></div></td></tr> :
                                        filtered.map((t: any) => (
                                            <tr key={t.id}>
                                                <td style={{ whiteSpace: 'nowrap' }}>{formatDateShort(t.transaction_date)}</td>
                                                <td style={{ fontWeight: 500, whiteSpace: 'nowrap' }}>{t.description}</td>
                                                <td style={{ whiteSpace: 'nowrap' }}>{t.program?.name || '-'}</td>
                                                <td style={{ whiteSpace: 'nowrap' }}>{t.category || '-'}</td>
                                                <td><span className={`badge badge-${t.type === 'income' ? 'success' : 'danger'}`} style={{ whiteSpace: 'nowrap' }}>{t.type === 'income' ? '↑ Masuk' : '↓ Keluar'}</span></td>
                                                <td style={{ fontWeight: 600, color: t.type === 'income' ? 'var(--color-success)' : 'var(--color-danger)', whiteSpace: 'nowrap' }}>{t.type === 'income' ? '+' : '-'}{formatCurrency(t.amount)}</td>
                                                <td>
                                                    <div style={{ display: 'flex', gap: 4 }}>
                                                        <button className="btn btn-ghost btn-sm" onClick={() => { setForm({ program_id: t.program_id || '', type: t.type, category: t.category || '', description: t.description, amount: t.amount.toString(), transaction_date: t.transaction_date, receipt_url: t.receipt_url || '' }); setEditId(t.id); setShowModal(true) }}>Edit</button>
