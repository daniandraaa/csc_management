...
import { Heart, Plus, Search, X, Upload, FileText, Download, ShieldAlert } from 'lucide-react'
...
export default function AdvocacyPage() {
    ...
    async function handleDelete(id: string) {
        if (confirm('Hapus data ini?')) {
            const { error } = await supabase.from('advocacy_aspirations').delete().eq('id', id)
    ...
    const exportData = filtered.map((i: any) => ({ ...i, _member: 'Anonim' }))
            ...
            <div className="page-container">
                ...
                <div className="toolbar">
                    <div className="toolbar-left">
                        <div className="search-input"><Search /><input className="form-input" placeholder="Cari judul..." value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: '2.25rem' }} /></div>
                    ...
                    <div className="toolbar-right">
                        {canEdit && <button className="btn btn-secondary btn-sm" onClick={() => setShowCsvImport(true)}><Upload size={14} /> Import CSV</button>}
                        ...
                        {canEdit && <button className="btn btn-primary" onClick={() => { setEditId(null); setForm({ member_id: '', title: '', description: '', category: 'advocacy', status: 'pending', response: '' }); setShowModal(true) }}><Plus size={16} /> Tambah</button>}
                    </div>
                </div>

                {isRestricted && currentUser?.role !== 'BOE' && (
                    <div style={{ marginBottom: '1rem', padding: '0.75rem', borderRadius: 'var(--radius-md)', background: 'var(--color-warning-bg)', color: 'var(--color-warning)', fontSize: '0.8125rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <ShieldAlert size={16} /> Anda tidak memiliki izin untuk mengedit atau merespon pengaduan ini.
                    </div>
                )}
                    ...
                    {loading ? <p style={{ padding: '2rem', textAlign: 'center' }}>Memuat...</p> :
                        filtered.length === 0 ? <div className="card"><div className="empty-state"><Heart size={48} /><h3>Belum ada data</h3><p>Tambahkan advokasi atau aspirasi baru.</p></div></div> :
                            filtered.map((item: any) => (
                                <div key={item.id} className="card" style={{ cursor: canEdit ? 'pointer' : 'default' }} onClick={() => { if (!canEdit) return; setForm({ member_id: item.member_id, title: item.title, description: item.description, category: item.category, status: item.status, response: item.response || '' }); setEditId(item.id); setShowModal(true) }}>
                                ...
                                <div className="modal-body">
                                    {!editId && (
                                        <div className="form-group">
                                            <label className="form-label">Anggota (Sumber) *</label>
                                            ...
                                            <p style={{ fontSize: '0.6875rem', color: 'var(--color-text-tertiary)', marginTop: '0.25rem' }}>ID anggota hanya digunakan untuk database dan tidak akan ditampilkan di daftar.</p>
                                        </div>
                                    )}
