...
export default function CounselingPage() {
    ...
    async function loadData() {
        ...
        const { data } = await supabase
            ...
            .select('*, member:members!counseling_requests_member_id_fkey(id,full_name), counselor:members!counseling_requests_counselor_id_fkey(id,full_name)')
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (editId) { 
            await supabase.from('counseling_requests').update(payload).eq('id', editId) 
        } else { 
    ...
    const exportData = filtered.map((r: any) => ({ 
        ...r, 
        _member: r.member?.full_name || '-', 
        _counselor: r.counselor?.full_name || '-' 
    }))
            ...
            <div className="page-container">
                ...
                <div className="stats-grid">
                    <div className="stat-card">
                        <div className="stat-icon" style={{ background: 'var(--color-warning-bg)', color: 'var(--color-warning)' }}><CalendarClock size={20} /></div>
                        <div>
                            <div className="stat-value" style={{ color: 'var(--color-warning)' }}>{requests.filter(r => r.status === 'pending').length}</div>
                            <div className="stat-label">Pending</div>
                        </div>
                    </div>
                    <div className="stat-card">
                        <div className="stat-icon" style={{ background: 'var(--color-info-bg)', color: 'var(--color-info)' }}><CalendarClock size={20} /></div>
                        <div>
                            <div className="stat-value" style={{ color: 'var(--color-info)' }}>{requests.filter(r => r.status === 'scheduled').length}</div>
                            <div className="stat-label">Terjadwal</div>
                        </div>
                    </div>
                    <div className="stat-card">
                        <div className="stat-icon" style={{ background: 'var(--color-success-bg)', color: 'var(--color-success)' }}><CalendarClock size={20} /></div>
                        <div>
                            <div className="stat-value" style={{ color: 'var(--color-success)' }}>{requests.filter(r => r.status === 'completed').length}</div>
                ...
                <div className="toolbar">
                    ...
                    <div className="toolbar-right">
                        ...
                        <button className="btn btn-primary" onClick={() => { 
                            setEditId(null); 
                            setForm({ member_id: '', subject: '', description: '', preferred_date: '', preferred_time: '', status: 'pending', scheduled_date: '', scheduled_time: '', counselor_id: '', notes: '' }); 
                            setShowModal(true) 
                        }}>
                            <Plus size={16} /> Request Konseling
                        </button>
                    </div>
                </div>
                                ...
                                <tr>
                                    <td colSpan={7}>
                                        <div className="empty-state">
                                            ...
                                            <p>Buat request konseling baru.</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                    ...
                    <div className="modal-overlay" onClick={() => setShowModal(false)}>
                        <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 640 }}>
                            <div className="modal-header">
                                <h2>{editId ? 'Edit Konseling' : 'Request Konseling'}</h2>
                                <button className="btn btn-ghost btn-icon" onClick={() => setShowModal(false)}><X size={18} /></button>
                            </div>
                            <form onSubmit={handleSubmit}>
                                <div className="modal-body">
                                    <div className="form-group">
                                        <label className="form-label">Anggota {editId ? '' : '*'}</label>
                                        <select 
                                        ...
                                        >
                                            <option value="">{editId ? 'Anonim (Tetap Anonim)' : 'Pilih'}</option>
                                    ...
                                    {editId && <>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                            ...
                                            <div className="form-group">
                                                <label className="form-label">Jadwal Jam</label>
                                                <input className="form-input" type="time" value={form.scheduled_time} onChange={e => setForm({ ...form, scheduled_time: e.target.value })} />
                                            </div>
                                        </div>
                                        <div className="form-group">
                                            <label className="form-label">Konselor</label>
                                            <select className="form-select" value={form.counselor_id} onChange={e => setForm({ ...form, counselor_id: e.target.value })}>
