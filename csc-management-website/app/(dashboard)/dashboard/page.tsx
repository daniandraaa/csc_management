...
export default function DashboardPage() {
    ...
    useEffect(() => {
        async function loadDashboardData() {
            ...
            // 5. Recent Activities
            const logs: any[] = []
            const { data: rProg } = await supabase.from('programs').select('id, name, created_at').order('created_at', { ascending: false }).limit(2)
            const { data: rReim } = await supabase.from('reimbursements').select('id, title, status, created_at').order('created_at', { ascending: false }).limit(2)
            const { data: rDocs } = await supabase.from('documents').select('id, title, created_at').order('created_at', { ascending: false }).limit(2)
            const { data: rMemb } = await supabase.from('members').select('id, full_name, created_at').order('created_at', { ascending: false }).limit(2)
            
            rProg?.forEach(p => logs.push({ id: `p-${p.id}`, action: `Program baru: ${p.name}`, time: formatDateShort(p.created_at), type: 'info', rawTime: p.created_at }))
            rReim?.forEach(r => logs.push({ id: `r-${r.id}`, action: `Reimburse: ${r.title} (${r.status})`, time: formatDateShort(r.created_at), type: r.status === 'pending' ? 'warning' : 'success', rawTime: r.created_at }))
            rDocs?.forEach(d => logs.push({ id: `d-${d.id}`, action: `Dokumen baru: ${d.title}`, time: formatDateShort(d.created_at), type: 'info', rawTime: d.created_at }))
            rMemb?.forEach(m => logs.push({ id: `m-${m.id}`, action: `Anggota baru: ${m.full_name}`, time: formatDateShort(m.created_at), type: 'success', rawTime: m.created_at }))
